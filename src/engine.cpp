...
void Engine::drawFrame() {
  ...
  postArgpassBarrier.srcAccessMask =
      VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
  postArgpassBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT |
  ...
  VkImageMemoryBarrier offscreenToTransferBarrier{
      VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};
  ...
  blitRegion.dstOffsets[1] = {(int)swapchainImageExtent.width,
  ...
  VkImageMemoryBarrier offscreenBackToGeneralBarrier{
      VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};
  offscreenBackToGeneralBarrier.image = offscreenImages[currentFrame];
  offscreenBackToGeneralBarrier.oldLayout =
  ...
  VkSemaphore graphicsWaitSemaphores[] = {
      computeFinishedSemaphore[currentFrame],
  ...
  vkQueueSubmit(graphicsQueue, 1, &graphicsSubmitInfo,
...
void Engine::train(std::vector<Image> &images, std::vector<Camera> &cameras,
                   int32_t iterations, float lr, float beta1, float beta2,
  ...
  VkImage outContribution; // image which records the index of lastest gaussian
                           // contributed to pixel color
  VkImage gtImage;
  ...
  createBuffer(sizeof(float) * 16 * maxGaussians,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, gaussianGradBuffer,
               gaussianGradBufferMemory);
  ...
  createBuffer(sizeof(float) * 2 * maxGaussians,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, gradPos2dBuffer,
               gradPos2dBufferMemory);

  VkBuffer adamMoments;
  VkDeviceMemory adamMomentsMemory;
  createBuffer(
      sizeof(AdamState) * maxGaussians,
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
          VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, adamMoments, adamMomentsMemory);

  VkBuffer flagBuffer;
  VkDeviceMemory flagBufferMemory;
  createBuffer(
      sizeof(uint32_t) * maxGaussians,
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
          VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, flagBuffer, flagBufferMemory);

  VkBuffer activeGaussianCountBuffer; // for density control, to prevent invalid
                                      // access during density control
  VkDeviceMemory activeGaussianCountBufferMemory;
  createBuffer(sizeof(uint32_t),
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, activeGaussianCountBuffer,
               activeGaussianCountBufferMemory);

  createBuffer(sizeof(uint32_t) * maxGaussians,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, contributionCountBuffer,
               contributionCountBufferMemory);
  ...
  createBuffer(
      ...
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
          VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, scanBuffer, scanBufferMemory);
  ...
  createBuffer(sizeof(uint32_t) * sum1Size,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, blockSumBuffer1,
               blockSumBuffer1Memory);
  ...
  createBuffer(sizeof(uint32_t) * sum2Size,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, blockSumBuffer2,
               blockSumBuffer2Memory);
  ...
  createBuffer(sizeof(Gaussian3D) * maxGaussians,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                   VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, tmpGaussianBuffer,
               tmpGaussianBufferMemory);
  ...
  createBuffer(
      sizeof(AdamState) * maxGaussians,
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, tmpAdamBuffer, tmpAdamBufferMemory);

  VkBuffer dummyBuffer; // For scan L3 output
  VkDeviceMemory dummyBufferMemory;
  createBuffer(sizeof(uint32_t) * 512, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, dummyBuffer,
               dummyBufferMemory);

  createImage(render_width, render_height, VK_FORMAT_R32_UINT,
              VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, outContribution,
              outContributionMemory);
  createImage(render_width, render_height, VK_FORMAT_R8G8B8A8_UNORM,
              VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, gtImage, gtImageMemory);
  ...
  createImageView(VK_FORMAT_R32_UINT, outContribution,
                  outContributionImageView);
  createImageView(VK_FORMAT_R8G8B8A8_UNORM, gtImage, gtImageView);
  createImageView(VK_FORMAT_R32G32B32A32_SFLOAT, curSSIMStats,
                  curSSIMStatsImageView);
  createImageView(VK_FORMAT_R32G32_SFLOAT, gtSSIMStats, gtSSIMStatsImageView);
  createImageView(VK_FORMAT_R32G32B32A32_SFLOAT, tempSSIMStats,
                  tempSSIMStatsImageView);
  createImageView(VK_FORMAT_R32G32B32A32_SFLOAT, tempSSIMStats2,
                  tempSSIMStatsImageView2);
  ...
  // initialize buffers
  {
    ...
    vkCmdFillBuffer(clearCmd, activeGaussianCountBuffer, 0, VK_WHOLE_SIZE,
                    totalGaussians);
  ...
  VkDescriptorImageInfo outContributionInfo{
      ...
      .imageLayout = VK_IMAGE_LAYOUT_GENERAL};
  VkDescriptorImageInfo curSSIMStatsInfo{.sampler = VK_NULL_HANDLE,
                                         .imageView = curSSIMStatsImageView,
                                         .imageLayout =
                                             VK_IMAGE_LAYOUT_GENERAL};
  VkDescriptorImageInfo gtSSIMStatsInfo{.sampler = VK_NULL_HANDLE,
                                        .imageView = gtSSIMStatsImageView,
                                        .imageLayout = VK_IMAGE_LAYOUT_GENERAL};
  VkDescriptorImageInfo tempSSIMStatsInfo{.sampler = VK_NULL_HANDLE,
                                          .imageView = tempSSIMStatsImageView,
                                          .imageLayout =
                                              VK_IMAGE_LAYOUT_GENERAL};
  VkDescriptorImageInfo tempSSIMStatsInfo2{.sampler = VK_NULL_HANDLE,
  ...
  std::array<VkDescriptorSetLayout, 2> trainSetLayouts = {
      globalDescriptorSetLayout, trainSetLayout};
  VkDescriptorSet bindTrainDescriptorSets[] = {globalDescriptorSets,
                                               trainDescriptorSet};
  ...
  createComputePipeline(
      rasterTrainComputePipeline, rasterTrainComputePipelineLayout,
  ...
  createComputePipeline(convComputePipeline, convComputePipelineLayout,
                        ...
                        static_cast<uint32_t>(trainSetLayouts.size()),
                        trainSetLayouts.data());

  VkPipeline convBackComputePipeline;
  VkPipelineLayout convBackComputePipelineLayout;
  createComputePipeline(
      convBackComputePipeline, convBackComputePipelineLayout,
  ...
  VkDescriptorSetLayout compactSpecificSetLayout;
  {
    std::array<VkDescriptorSetLayoutBinding, 2> bindings{};
    bindings[0] = {0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                   VK_SHADER_STAGE_COMPUTE_BIT, nullptr}; // dst gaussians
    bindings[1] = {1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                   VK_SHADER_STAGE_COMPUTE_BIT, nullptr}; // dst adam
    VkDescriptorSetLayoutCreateInfo info{
        VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO, nullptr, 0, 2,
        bindings.data()};
    vkCreateDescriptorSetLayout(device, &info, nullptr,
                                &compactSpecificSetLayout);
  }
  ...
  VkDescriptorSet scanSetL1, scanSetL2, scanSetL3, compactSet;
  {
    VkDescriptorSetAllocateInfo alloc{
        VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr, scanPool, 1,
        &scanSetLayout};
  ...
  createComputePipeline(scanLocalPipeline, scanLocalPipelineLayout,
                        "../shader/spv/scan_local.spv", 0, nullptr, 1,
                        &scanSetLayout);
  ...
  createComputePipeline(scanAddPipeline, scanAddPipelineLayout,
                        "../shader/spv/scan_add.spv", 0, nullptr, 1,
                        &scanSetLayout);
  ...
  createComputePipeline(compactPipeline, compactPipelineLayout,
                        "../shader/spv/compact.spv", 0, nullptr, 3,
                        compactLayouts.data());
  ...
  createComputePipeline(
      densityControlPipeline, densityControlPipelineLayout,
      "../shader/spv/density_control.spv", 1, &densityControlPushRange,
      static_cast<uint32_t>(trainSetLayouts.size()), trainSetLayouts.data());
  ...
  void *data;
  vkMapMemory(device, imageStagingBufferMemory, 0,
              render_width * render_height * 4, 0, &data);
  ...
  while (iterations--) {
    ...
    // --- Synchronization & Resets ---
    vkWaitForFences(device, 1, &computeInFlightFences[frameIdx], VK_TRUE,
                    UINT64_MAX);
    ...
    if (gtImageCache[imgIdx].empty()) {
      ...
      if (!pixels) {
        throw std::runtime_error("Failed to load ground truth image: " +
                                 imagePath);
      }
      ...
      if (texWidth != render_width || texHeight != render_height) {
        stbir_resize_uint8_linear(pixels, texWidth, texHeight, 0,
                                  gtImageCache[imgIdx].data(), render_width,
                                  render_height, 0, STBIR_RGBA);
      } else {
        memcpy(gtImageCache[imgIdx].data(), pixels,
    ...
    vkCmdCopyBufferToImage(cmdbuf, imageStagingBuffer, gtImage,
    ...
    vkCmdClearColorImage(cmdbuf, outContribution, VK_IMAGE_LAYOUT_GENERAL,
    ...
    vkCmdPipelineBarrier(cmdbuf, VK_PIPELINE_STAGE_TRANSFER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                         &initBarrier, 0, nullptr, 0, nullptr);
    ...
    passAFinalBarrier.dstAccessMask =
        VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT;
    vkCmdPipelineBarrier(cmdbuf, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 1,
                         &passAFinalBarrier, 0, nullptr, 0, nullptr);
    ...
    postSortBarrier.dstAccessMask = VK_ACCESS_MEMORY_READ_BIT |
    ...
    imageToGeneralBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
    imageToGeneralBarrier.srcAccessMask =
        VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
    imageToGeneralBarrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
    imageToGeneralBarrier.subresourceRange = subresourceRange;
    ...
    std::array<VkImageMemoryBarrier, 2> imageBarriers = {
        imageToGeneralBarrier, contribToGeneralBarrier};

    VkMemoryBarrier postRangeBarrier{VK_STRUCTURE_TYPE_MEMORY_BARRIER};
    postRangeBarrier.srcAccessMask =
        VK_ACCESS_MEMORY_WRITE_BIT | VK_ACCESS_MEMORY_READ_BIT;
    postRangeBarrier.dstAccessMask =
        VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT;
    vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                         &postRangeBarrier, 0, nullptr,
                         (uint32_t)imageBarriers.size(), imageBarriers.data());

    vkCmdBindPipeline(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
                      rasterTrainComputePipeline);
    // Synthetic datasets (Lego) require a White Background to compel identical
    // geometric opacity. Against a Black background, dark structural portions
    // trivially survive as transparent 'ghosts' without incurring loss. White
    // forces solid boundary learning. Set training background to match the GT
    // baked-in Background (Black) 1.0 (White) vs Black GT Forces the model to
    // spawn infinite Black Gaussians to cover the White void.
    float bgR = 0.0f;
    float bgG = 0.0f;
    float bgB = 0.0f;
    RasterPush rasterPush{
        KVCapacity,           bgR, bgG, bgB, (uint32_t)n_tiles_col,
        (uint32_t)n_tiles_row};
    ...
    preConvBarrier.srcAccessMask =
        VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT;
    preConvBarrier.dstAccessMask =
        VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT;
    vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
    ...
    vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                         &midConvBarrier, 0, nullptr, 0, nullptr);
    ...
    vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                         &postConvBarrier, 0, nullptr, 0, nullptr);

    // --- SSIM Backward Pass (Horizontal & Vertical) ---
    vertical = 0;
    vkCmdBindPipeline(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
                      convBackComputePipeline);
    ...
    vkCmdBindPipeline(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
                      backwardComputePipeline);
    active_sh_degree = std::min(3u, steps / 1000u);
    BackwardPush backPush{lambda,
                          bgR,
                          bgG,
                          bgB,
                          KVCapacity,
                          (uint32_t)n_tiles_col,
                          (uint32_t)n_tiles_row,
                          active_sh_degree};
    ...
    vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                         &preAdamBarrier, 0, nullptr, 0, nullptr);
    ...
    if (steps % 100 == 0) {
      ...
      vkCmdCopyBuffer(cmdbuf2, gaussianCountBuffer, activeGaussianCountBuffer,
                      1, &bufferRegionInit);
      ...
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_TRANSFER_BIT,
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT |
                               VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT,
                           0, 1, &preDensityControlBarrier, 0, nullptr, 0,
                           nullptr);
      ...
      postDensityBarrier.dstAccessMask =
          VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                           &postDensityBarrier, 0, nullptr, 0, nullptr);
      ...
      vkCmdPushConstants(cmdbuf2, markPrunePipelineLayout,
      ...
      // Current maxGaussians for pruning (not active_count as we might have
      // added new ones)
      ...
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &markBarrier,
                           0, nullptr, 0, nullptr);
      ...
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                           &scanL1Barrier, 0, nullptr, 0, nullptr);
      ...
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                           &scanL2Barrier, 0, nullptr, 0, nullptr);

      // Level 3
      vkCmdBindDescriptorSets(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
                              scanLocalPipelineLayout, 0, 1, &scanSetL3, 0,
                              nullptr);
      vkCmdDispatch(cmdbuf2, 1, 1, 1);
      ...
      vkCmdPipelineBarrier(cmdbuf2, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1,
                           &addL2Barrier, 0, nullptr, 0, nullptr);
      ...
      vkCmdBindPipeline(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
                        compactPipeline);
      vkCmdFillBuffer(cmdbuf2, gaussianCountBuffer, 0, sizeof(uint32_t),
                      0); // Reset counter for atomicMax
      ...
      VkDescriptorSet compactBindSets[] = {globalDescriptorSets,
                                           trainDescriptorSet, compactSet};
      vkCmdBindDescriptorSets(cmdbuf2, VK_PIPELINE_BIND_POINT_COMPUTE,
    ...
    vkQueueSubmit(computeQueue, 1, &submitInfoB,
                  computeInFlightFences[frameIdx]);
  }
  ...
  // Sync Gaussian count from GPU to CPU
  {
    ...
    void *pCountData;
    vkMapMemory(device, countStagingBufferMemory, 0, sizeof(uint32_t), 0,
                &pCountData);
  ...
  if (totalGaussians > maxGaussians) {
    printf("Warning: totalGaussians (%u) exceeded maxGaussians (%u). Clamping "
           "to max.\n",
  ...
  Core::exportGaussians("trained_gaussians.bin", exportedGaussians,
                        totalGaussians);
...
Engine::Engine(uint64_t src_width, uint64_t src_height, float scale,
  ...
  std::vector<Gaussian3D> src_tmp =
      gaussianFromPoints(points, totalGaussians, maxGaussians);

  createStorageBuffer<Gaussian3D>(src_tmp, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
               ...
               gaussianCountBufferMemory);
  {
    ...
    vkCmdFillBuffer(cmdbuf, gaussianCountBuffer, 0, sizeof(uint32_t),
                    totalGaussians);
  ...
  createStorageBuffer<TileRange>(num_tiles, tileRangeBuffer,
                                 tileRangeBufferMemory);
  ...
  for (int i = 0; i < MAX_FRAME_IN_FLIGHT; i++) {
    createUniformBuffer<CameraUBO>(cameraBuffers[i], cameraBufferMemory[i],
    ...
    createImage(render_width, render_height, VK_FORMAT_R8G8B8A8_UNORM,
                ...
                imageMemory[i]);
    createImageView(VK_FORMAT_R8G8B8A8_UNORM, offscreenImages[i],
  ...
  std::array<VkDescriptorSetLayout, 2> setLayout = {globalDescriptorSetLayout,
...
void Engine::initWindow() {
  ...
  pWindow =
      glfwCreateWindow(DEFAULT_WIDTH, DEFAULT_HEIGHT, "3DGS", nullptr, nullptr);
}
...
void Engine::createLogicalDevice() {
  ...
  if (vkCmdPushDescriptorSetKHR_ptr == nullptr) {
    fprintf(stderr, "[Critical] vkCmdPushDescriptorSetKHR_ptr is NULL! Radix "
                    "sort WILL NOT WORK.\n");
  } else {
    fprintf(stderr,
            "[Info] vkCmdPushDescriptorSetKHR_ptr successfully loaded.\n");
  }
  if (!vkCmdPushDescriptorSetKHR_ptr) {
    throw std::runtime_error(
        "Found no vkCmdPushDescriptorSetKHR function pointer!");
  }
...
void Engine::createSwapchain() {
  ...
  if (surfaceCaps.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
    ...
    imageExtent.height =
        std::clamp<int>(width, surfaceCaps.minImageExtent.height,
  ...
  vkGetSwapchainImagesKHR(device, swapchain, &minImageCount,
...
void Engine::createSorterAndBuffer() {
  ...
  KVCapacity = maxGaussians
               << 8; // static pre-allocation pool (reverted from 10 to 8 to fix
                     // Apple M1 Out of Memory crash)
  ...
  VkBufferUsageFlags kvUsage = reqs.usage | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                               VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                               VK_BUFFER_USAGE_TRANSFER_SRC_BIT;

  createBuffer(kvBufferSize, kvUsage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               keyBuffer, keyBufferMemory);
  createBuffer(kvBufferSize, kvUsage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               valueBuffer, valueBufferMemory);
...
void Engine::createSyncObjects() {
  ...
  for (int i = 0; i < MAX_FRAME_IN_FLIGHT; i++) {
    ...
    vkCreateSemaphore(device, &semaInfo, nullptr,
                      &projectionFinishedSemaphores[i]);
...
void Engine::createDescriptorSetLayouts() {
  ...
  vkCreateDescriptorSetLayout(device, &setLayoutInfo, nullptr,
                              &globalDescriptorSetLayout);
...
void Engine::createDescriptorSets() {
  ...
  vkUpdateDescriptorSets(device, descriptorWriteGlobal.size(),
                         descriptorWriteGlobal.data(), 0, nullptr);
...
void Engine::createImageView(VkFormat format, VkImage &image,
...
void Engine::createUniformBuffer(VkBuffer &buffer, VkDeviceMemory &bufferMemory,
                                 void *&pData) {
...
template <typename T>
void Engine::createStorageBuffer(size_t num_elements, VkBuffer &buffer,
                                 VkDeviceMemory &bufferMemory) {
...
template <typename T>
void Engine::createStorageBuffer(std::vector<T> &srcBuffer, VkBuffer &buffer,
                                 VkDeviceMemory &bufferMemory) {
...
void Engine::updateCameraUBO(float deltaTime) {
  ...
  // Translation (W/S for Forward/Backward, A/D for Strafe, SPACE/LSHIFT for
  // Vertical relative to local Up)
  ...
  updateCamera(camUBO, cameraPos, cameraPos + cameraFront, render_width,
               render_height, cameraUp);
